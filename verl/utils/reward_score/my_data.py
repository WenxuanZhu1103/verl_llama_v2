...
def compute_score(model_output: str, ground_truth: str, timeout_score=0) -> tuple[bool, float]:
    with suppress_math_verify_logs():
        ...
        extracted_output = "\\boxed{" + extracted_output + "}"
        try:
            verify_func = math_metric(
                gold_extraction_target=(LatexExtractionConfig(),),
                pred_extraction_target=(ExprExtractionConfig(), LatexExtractionConfig()),
            )
            ret_score, _ = verify_func([ground_truth_boxed], [extracted_output])
        except TimeoutException:
            # print("Math-Verify TimeoutException: ", model_output[-100:])
            return False, timeout_score
        except Exception as e:
            # print(f"Error in math-verify score: {e} Model output: {model_output[-100:]}")
            return False, timeout_score
        return True, ret_score
