...
@register_adv_est(AdvantageEstimator.MY_GRPO)  # or simply: @register_adv_est("my_grpo")
def compute_my_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    token_level_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    config: Optional[AlgoConfig] = None,
    ...
    """
    Compute advantage for MY_GRPO, operating only on Outcome reward
    (with only one scalar reward for each response).
    """
    scores = token_level_rewards.sum(dim=-1)
    id2score = defaultdict(list)
    ...
    with torch.no_grad():
        ...
        scores = scores.unsqueeze(-1) * response_mask * token_level_mask
    # breakpoint()
    return scores, scores
...
def compute_policy_loss(
    ...
    importance_sampling_mode: str = "vanilla", # vanilla, step, response
    ...
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=10.0)
    
    if importance_sampling_mode == "vanilla":
        ratio = torch.exp(negative_approx_kl)
    elif importance_sampling_mode == "step_avg":
        new_line_token_ids = torch.tensor([512, 1473, 5380, 198, 4999, 1980, 271, 720, 627, 220, 2268, 382], device=responses.device)
        # 2) 基础张量
        ratio = torch.exp(negative_approx_kl)     # [B, S]
        B, S = ratio.shape
        step_ratio = ratio.clone()                 # clone() 会保留梯度依赖（CloneBackward）

        # 3) 右填充：有效长度就是 mask 中 True 的个数；有效区间为 [0:L)
        valid_len = response_mask.sum(dim=1)      # [B], long/int
        ...
        for b in range(B):
            ...
            tokens_b = responses[b, :L]           # [L] 仅看有效部分
            ratio_b  = ratio[b, :L]               # [L]

            # 找到所有换行符的位置
            nl_pos = []
            for nl_id in new_line_token_ids:
                pos = (tokens_b == nl_id).nonzero(as_tuple=True)[0]
                if len(pos) > 0:
                    nl_pos.append(pos)
            
            # 合并所有换行符位置并排序
            if len(nl_pos) > 0:
                nl_pos = torch.cat(nl_pos)
                nl_pos = torch.unique(nl_pos, sorted=True)
            else:
                nl_pos = torch.tensor([], device=tokens_b.device)

            # 构造 step 边界：起点0、每个换行后的下一个位置、终点L
            boundaries = torch.cat([
                torch.tensor([0], device=tokens_b.device),
                nl_pos + 1,
                torch.tensor([L], device=tokens_b.device)
            ])
            boundaries = torch.unique(boundaries, sorted=True)

            # 逐 step 求均值并写回（直通估计：值替换，梯度保留）
            breakpoint()
            for i in range(len(boundaries) - 1):
                ...
                step_avg = ratio_b[s:e].mean()    # 标量

                x = step_ratio[b, s:e]            # 原片段（保留梯度路径）
                step_ratio[b, s:e] = step_avg.detach() + x - x.detach()

        # 4) 返回结果
        ratio = step_ratio
        
    elif importance_sampling_mode == "response_avg":
        seq_lengths = torch.sum(response_mask, dim=-1).clamp(min=1)
        ratio_sum = torch.sum(torch.exp(negative_approx_kl) * response_mask, dim=-1)
        ratio_avg = ratio_sum / seq_lengths
        ratio = log_prob.exp() - log_prob.exp().detach() + ratio_avg.detach().unsqueeze(-1)
    elif importance_sampling_mode == "response_multiply":
        seq_lengths = torch.sum(response_mask, dim=-1).clamp(min=1)
        ...
        raise ValueError(f"Invalid importance_sampling_mode: {importance_sampling_mode}")
