...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @DistProfiler.annotate(color="red", role="rollout_check_sequences")
    def check_sequences(self, prompts: DataProto):
        # Support all hardwares
        prompts = prompts.to(get_device_id())
        ...
        with self.rollout_sharding_manager:
            ...
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            with simple_timer("check_sequences", timing_generate):
                output = self.rollout.check_sequences(prompts=prompts, tokenizer=self.tokenizer)
            # breakpoint()

            log_gpu_memory_usage("After rollout check_sequences", logger=logger)
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            prompt_with_chat_template = target_tokenizer.apply_chat_template(
                chat, add_generation_prompt=False, tokenize=False, enable_thinking=self.config.enable_thinking
            )
